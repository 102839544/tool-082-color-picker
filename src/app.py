...
class App:
    ...
    def setup_ui(self):
        ...
        tk.Label(title_frame, text="📦 颜色选择器", font=("Arial", 18, "bold"),
        ...
        # 根据工具名称添加特定功能
        if "timer" in "color-picker".lower() or "clock" in "color-picker".lower():
            self.setup_timer_ui(main)
        elif "random" in "color-picker".lower() or "dice" in "color-picker".lower() or "coin" in "color-picker".lower():
            self.setup_random_ui(main)
        elif "note" in "color-picker".lower() or "todo" in "color-picker".lower():
    ...
    def generate_random(self):
        if "dice" in "color-picker".lower():
            result = random.randint(1, 6)
            self.result_label.config(text=f"🎲 {result}")
        elif "coin" in "color-picker".lower():
